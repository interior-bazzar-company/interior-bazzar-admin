...
| **TM-T30 · Edit an item** | `PATCH /admin/team/work/{id}` | Accepts `title`, `assigneeId`, `priority`, `startDate`, `dueDate`, `parentId` — and **not `kind`**: a kind decides what may sit under an item, so changing it would orphan children silently. Refusals the client already gives and the server must repeat: assignee inactive; a task cannot hold children; a target is always top level; a milestone rolls up to a target; the new parent may not already roll up to this item; not due before it starts. Reassigning drops the tags the last assignee owned, because a tag is a member's record. Added 2026-09-07 — nothing was editable after creation before this. |
| **TM-T31 · Restore** | `POST /admin/team/work/{id}/status` with `planned` | Not new: the vocabulary has defined cancelled → planned ("Restore", reason required) since Module 7. The panel simply had no button for it until 2026-09-07. Listed so the transition is not dropped as unused. |
| **TM-T40 · An agreement was opened** | `POST /admin/team/agreements/{id}/viewed` | Idempotent; only from `sent`, sets `viewedAt` and `state: viewed`. The public `/sign/{token}` GET may do this itself — either way it is the server's stamp, never the client's. Until 2026-09-07 `viewedAt` was read in four places and written in none. |
| **TM-T50 · Replace a required document** | `PUT /admin/team/members/{id}/documents/{kind}` | A **required** kind is one record per member: a second upload of the same kind replaces it in place, keeps the id, and resets `verifiedById`/`verifiedAt` because nobody has checked the new file. Non-required kinds may repeat. The panel appended a duplicate and showed the stale one until 2026-09-07. |
...
| Item | Where | What has to happen |
| --- | --- | --- |
| Team reads pay from Finance | server | `#/team/:id/pay` reads `GET /admin/finance/salary-accounts?memberId=` and that account's slips — it read Team's own `pay.json` until 2026-09-07, the "two engines, one slip" this list forbids. Incentives stay Team's. A member with no Finance account is an empty state, not an error. |
