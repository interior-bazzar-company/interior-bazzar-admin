...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    /* RAC 1.21 types onChange for a single date OR a list; this provider only ever holds one. */
    const onChange = (v: DateValue | readonly DateValue[] | null) => setValue(Array.isArray(v) ? ((v[0] as DateValue) ?? null) : (v as DateValue | null));
